  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: '1',
      sender: 'bot',
      text: "Hello! I am TrueStyle AI. How can I help protect your shopping experience today?",
      timestamp: new Date()
    }
  ]);
    ...
    <div className="fixed bottom-6 right-6 z-40 select-none">
      ...
      {!isOpen && (
        <button
          ...
          className="flex items-center justify-center w-14 h-14 bg-accent hover:bg-accent rounded-full shadow-neon-blue transition duration-300 text-foreground transform hover:scale-105"
        >
      ...
      {/* Chat Window Panel */}
      {isOpen && (
        <div className="w-80 sm:w-96 h-[480px] bg-card border border-border rounded-xl shadow-glass flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-6">
          {/* Header */}
          <div className="bg-accent/80 px-4 py-3 border-b border-border flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <div className="bg-accent p-1 rounded-full text-foreground">
                <Shield className="w-4 h-4" />
              </div>
              <div>
                <h4 className="text-sm font-semibold text-foreground font-mono">TrueStyle AI</h4>
                <div className="flex items-center space-x-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-ping"></span>
                  <span className="text-[10px] text-muted">Monitoring Active</span>
                </div>
              </div>
            </div>
            <button 
          ...
          <div className="flex-grow p-4 overflow-y-auto space-y-4">
            {messages.map((msg) => (
              ...
              >
                <div className={`p-1.5 rounded-full ${msg.sender === 'bot' ? 'bg-accent text-accent' : 'bg-gray-800 text-muted'}`}>
                ...
                <div className={`max-w-[75%] p-3 rounded-lg text-xs leading-relaxed ${
                  msg.sender === 'bot' 
                    ? 'bg-cyber-dark-bg text-muted border border-border' 
                    : 'bg-accent text-foreground shadow-neon-blue'
                }`}>
                  {msg.text}
                </div>
              </div>
          ...
          {/* Form Input */}
          <form onSubmit={handleSend} className="p-3 border-t border-border bg-background flex space-x-2">
            <input
              type="text"
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              placeholder="Ask about price rules, scanner logic..."
              className="flex-grow glass-input px-3 py-2 rounded-lg text-xs text-foreground"
            />
            <button
              type="submit"
              className="px-3 bg-accent hover:bg-accent text-foreground rounded-lg transition"
            >
