...
2. **Supabase Dual-Mode**: Operates on live Supabase connections if credentials are provided in `.env`. Otherwise, it falls back to a persistent **Simulated Local State Mode** stored in `localStorage`.
...
- `src/context/AuthContext.tsx` - Handles registration complexity, captcha checks, and Supabase integration.
