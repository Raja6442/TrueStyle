...
const DEFAULT_BRANDS: OfficialBrand[] = [
  { id: 'brand-1', brand_name: 'Nike', official_domains: ['nike.com', 'nike.co.jp', 'nike.ae'], category: 'Sportswear', status: 'verified', updated_at: '2026-07-01T12:00:00Z' },
  { id: 'brand-2', brand_name: 'Gucci', official_domains: ['gucci.com', 'gucci.cn'], category: 'Luxury', status: 'verified', updated_at: '2026-07-02T12:00:00Z' },
  { id: 'brand-3', brand_name: 'Louis Vuitton', official_domains: ['louisvuitton.com'], category: 'Luxury', status: 'verified', updated_at: '2026-07-03T12:00:00Z' },
  { id: 'brand-4', brand_name: 'Adidas', official_domains: ['adidas.com', 'adidas.co.uk'], category: 'Sportswear', status: 'verified', updated_at: '2026-07-04T12:00:00Z' },
...
class MockDatabase {
  ...
  public getScans(userId?: string): ProductScan[] {
    const all = this.getStorage<ProductScan[]>('scans', DEFAULT_SCANS);
    if (userId) {
  ...
  public getFavorites(userId: string): FavoriteBrand[] {
    return this.getStorage<FavoriteBrand[]>('favorites', DEFAULT_FAVORITES).filter(f => f.user_id === userId);
  }
  ...
  public getBookmarks(userId: string): BookmarkedProduct[] {
    return this.getStorage<BookmarkedProduct[]>('bookmarks', DEFAULT_BOOKMARKS).filter(b => b.user_id === userId);
  }
