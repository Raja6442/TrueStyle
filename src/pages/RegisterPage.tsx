    ...
    <div className="max-w-md mx-auto px-4 py-12">
      <GlassCard className="p-8 border-cyber-blue-900/50 shadow-neon-blue">
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-white font-mono tracking-wide uppercase">Register Node</h2>
          <p className="text-xs text-gray-500 mt-1 font-mono">TrueStyle Cyber Registry Access</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full glass-input pl-3.5 pr-10 py-2.5 rounded-lg text-xs text-white font-mono"
          ...
          {password.length > 0 && (
            <div className="p-3 bg-cyber-dark-bg border border-cyber-dark-border rounded-lg grid grid-cols-2 gap-2 text-[10px]">
              <div className={`flex items-center space-x-1.5 ${rules.length ? 'text-green-400' : 'text-gray-500'}`}>
                {rules.length ? <Check className="w-3.5 h-3.5" /> : <X className="w-3.5 h-3.5 text-red-500" />}
                <span>8+ Characters</span>
              </div>
              <div className={`flex items-center space-x-1.5 ${rules.upper ? 'text-green-400' : 'text-gray-500'}`}>
                {rules.upper ? <Check className="w-3.5 h-3.5" /> : <X className="w-3.5 h-3.5 text-red-500" />}
                <span>1 Uppercase</span>
              </div>
              <div className={`flex items-center space-x-1.5 ${rules.lower ? 'text-green-400' : 'text-gray-500'}`}>
                {rules.lower ? <Check className="w-3.5 h-3.5" /> : <X className="w-3.5 h-3.5 text-red-500" />}
                <span>1 Lowercase</span>
              </div>
              <div className={`flex items-center space-x-1.5 ${rules.number ? 'text-green-400' : 'text-gray-500'}`}>
                {rules.number ? <Check className="w-3.5 h-3.5" /> : <X className="w-3.5 h-3.5 text-red-500" />}
                <span>1 Number</span>
              </div>
              <div className={`flex items-center space-x-1.5 ${rules.special ? 'text-green-400' : 'text-gray-500'}`}>
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-400 mb-1">Confirm Access Token</label>
            <input
              type="password"
              value={confirmPassword}
              onChange={(e) => setConfirmPassword(e.target.value)}
              className="w-full glass-input px-3.5 py-2.5 rounded-lg text-xs text-white font-mono"
          ...
          {/* Math CAPTCHA Form */}
          <div className="bg-cyber-dark-bg p-3 border border-cyber-dark-border rounded-lg space-y-2 select-none">
            ...
            <div className="flex items-center justify-between">
              <span className="text-xs text-white font-mono">
                Solve puzzle: <span className="font-bold text-cyber-blue-400">{num1} + {num2}</span> = ?
              </span>
              <input
                type="number"
                value={captchaAnswer}
                onChange={(e) => setCaptchaAnswer(e.target.value)}
                className="w-20 text-center glass-input px-2 py-1.5 rounded-md text-xs text-white"
                placeholder="Result"
                required
              />
            </div>
          ...
          <label className="flex items-start text-[10px] text-gray-400 select-none cursor-pointer">
            <input
              type="checkbox"
              checked={acceptTerms}
              onChange={(e) => setAcceptTerms(e.target.checked)}
              className="rounded bg-cyber-dark-bg border-cyber-dark-border text-cyber-blue-600 focus:ring-cyber-blue-500 mr-2 mt-0.5"
            />
            <span>
              I accept the{' '}
              <Link to="/terms" target="_blank" className="text-cyber-blue-400 hover:underline">
                Terms of Service
              </Link>{' '}
              and{' '}
              <Link to="/privacy" target="_blank" className="text-cyber-blue-400 hover:underline">
                Privacy Policy
              </Link>
              .
            </span>
        ...
        <div className="mt-6 pt-6 border-t border-cyber-dark-border text-center">
          <p className="text-xs text-gray-400">
            Registered terminal?{' '}
            <Link to="/login" className="text-cyber-blue-400 hover:text-white font-semibold transition">
              Log In
            </Link>
          </p>
        </div>
