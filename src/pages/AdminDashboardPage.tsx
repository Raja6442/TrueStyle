...
export const AdminDashboardPage: React.FC = () => {
  ...
  const toggleUserRole = async (targetUser: Profile) => {
    ...
    const updated = { ...targetUser, role: nextRole };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      {/* Header Banner */}
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 select-none border-b border-cyber-dark-border pb-6">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1 font-mono">SECURE NODE HOST: {user.email} (Level: Root Admin)</p>
        </div>

        {/* Tab switch buttons */}
        <div className="flex flex-wrap gap-2 mt-4 md:mt-0 bg-cyber-dark-card border border-cyber-dark-border p-1 rounded-lg">
          {[
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs font-semibold uppercase tracking-wider flex items-center transition ${
                adminTab === tab.id ? 'bg-red-700 text-white shadow-neon-blue' : 'text-gray-400 hover:text-white'
              }`}
            >
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 select-none">
            <GlassCard hoverable={false} className="p-4 border-red-950/30">
              <span className="text-[10px] text-gray-500 uppercase tracking-widest font-mono">Total Registry Users</span>
              <div className="text-2xl font-bold text-white font-mono mt-1">{totalUsersCount}</div>
            </GlassCard>

            <GlassCard hoverable={false} className="p-4 border-red-950/30">
              <span className="text-[10px] text-gray-500 uppercase tracking-widest font-mono">Official Brand Scans</span>
              <div className="text-2xl font-bold text-cyber-blue-400 font-mono mt-1">{verifiedBrandsCount}</div>
            </GlassCard>

            <GlassCard hoverable={false} className="p-4 border-red-950/30">
              <span className="text-[10px] text-gray-500 uppercase tracking-widest font-mono">Trusted / Flags Merchants</span>
            ...
            <GlassCard hoverable={false} className="p-4 border-red-950/30">
              <span className="text-[10px] text-gray-500 uppercase tracking-widest font-mono">Pending Support Tickets</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 select-none">
            ...
            <GlassCard hoverable={false} className="p-5 md:col-span-1 border-red-950/20">
              <h4 className="text-xs font-bold text-white font-mono uppercase tracking-wider mb-4">Security Recovery Keys</h4>
              <div className="space-y-3.5">
                ...
                <div className="p-3 bg-cyber-dark-bg border border-cyber-dark-border rounded text-[10px] text-gray-500 font-mono leading-relaxed">
            ...
            <GlassCard hoverable={false} className="p-5 md:col-span-2 border-red-950/20">
              <h4 className="text-xs font-bold text-white font-mono uppercase tracking-wider mb-4 flex items-center">
                <Terminal className="w-4 h-4 mr-1 text-red-500" />
                Live Heuristics Terminal Log
              </h4>
              <div className="bg-[#050608] border border-cyber-dark-border p-3.5 rounded-lg h-36 overflow-y-auto font-mono text-[9px] text-gray-400 space-y-1.5 scrollbar-thin">
                {logs.slice(0, 5).map(log => (
                  <div key={log.id} className="truncate">
                    <span className="text-red-500">&gt;</span> <span className="text-gray-500">[{new Date(log.created_at).toLocaleTimeString()}]</span> <span className="font-semibold text-white">{log.actor_name}</span>: {log.action} - {log.details}
                  </div>
                ))}
              </div>
            </GlassCard>
      ...
      {adminTab === 'users' && (
        <GlassCard hoverable={false} className="p-6">
          <h3 className="text-sm font-bold text-white font-mono uppercase tracking-wider mb-6 pb-2 border-b border-cyber-dark-border flex items-center">
            <Users className="w-4 h-4 mr-2 text-cyber-blue-500" />
            Registered Identity Nodes
          </h3>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-cyber-dark-border text-[10px] font-bold text-gray-500 font-mono uppercase tracking-widest bg-cyber-dark-bg/25">
              ...
              <tbody className="divide-y divide-cyber-dark-border text-xs">
                {users.map(u => (
                  <tr key={u.id} className="hover:bg-cyber-dark-border/20 transition-all">
                    <td className="py-3 px-4 font-mono text-[10px] text-gray-500 select-all">{u.id}</td>
                    <td className="py-3 px-4 font-semibold text-white">{u.full_name}</td>
                    <td className="py-3 px-4 font-mono text-gray-400 select-all">{u.email}</td>
                    <td className="py-3 px-4 text-gray-500 font-mono">
                      {new Date(u.created_at).toLocaleDateString()}
                    </td>
                    <td className="py-3 px-4 font-mono">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                        u.role === 'admin' ? 'bg-red-950 text-red-400' : 'bg-cyber-blue-950 text-cyber-blue-400'
                      }`}>
                        {u.role}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-right">
                      <button
                        ...
                        className="px-2.5 py-1 bg-cyber-dark-card hover:bg-cyber-dark-border border border-cyber-dark-border rounded text-[10px] text-gray-400 hover:text-white transition"
                      >
      ...
      {adminTab === 'brands' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <GlassCard hoverable={false} className="lg:col-span-1">
            <h3 className="text-sm font-bold text-white font-mono uppercase tracking-wider mb-6 pb-2 border-b border-cyber-dark-border">
            ...
            <form onSubmit={handleAddBrand} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1">Authorized Domains (comma-separated)</label>
                <input
                  type="text"
              ...
              <button
                type="submit"
                className="w-full py-2.5 rounded bg-cyber-blue-700 hover:bg-cyber-blue-600 text-white font-semibold text-xs tracking-wider uppercase transition shadow-neon-blue"
              >
          ...
          <GlassCard hoverable={false} className="lg:col-span-2">
            <h3 className="text-sm font-bold text-white font-mono uppercase tracking-wider mb-6 pb-2 border-b border-cyber-dark-border flex items-center select-none">
              <FolderLock className="w-4 h-4 mr-2 text-cyber-blue-500" />
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-cyber-dark-border text-xs">
                  {brands.map(b => (
                    <tr key={b.id} className="hover:bg-cyber-dark-border/20 transition-all">
                      <td className="py-3 px-4 font-semibold text-white">{b.brand_name}</td>
                      <td className="py-3 px-4 font-mono text-gray-400 select-all">
                        {b.official_domains.join(', ')}
                      </td>
                      <td className="py-3 px-4 text-gray-500 font-mono">{b.category}</td>
                      <td className="py-3 px-4 text-right">
                        <button
                          onClick={() => handleDeleteBrand(b.id, b.brand_name)}
                          className="p-1 text-gray-500 hover:text-red-500 rounded transition"
                          title="Purge brand"
                        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <GlassCard hoverable={false} className="lg:col-span-1">
            ...
            <form onSubmit={handleAddSeller} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1">Merchant Store Name</label>
                <input
                  type="text"
                  value={newSellerName}
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1">Host Platform</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1">Status Type</label>
                <select
          ...
          <GlassCard hoverable={false} className="lg:col-span-2">
            <h3 className="text-sm font-bold text-white font-mono uppercase tracking-wider mb-6 pb-2 border-b border-cyber-dark-border flex items-center select-none">
              <Store className="w-4 h-4 mr-2 text-cyber-blue-500" />
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-cyber-dark-border text-xs">
                  {sellers.map(s => (
                    <tr key={s.id} className="hover:bg-cyber-dark-border/20 transition-all">
                      <td className="py-3 px-4 font-semibold text-white">{s.seller_name}</td>
                      <td className="py-3 px-4 text-gray-400 font-mono select-all">{s.platform}</td>
      ...
      {adminTab === 'tickets' && (
        <GlassCard hoverable={false} className="p-6">
          <h3 className="text-sm font-bold text-white font-mono uppercase tracking-wider mb-6 pb-2 border-b border-cyber-dark-border flex items-center select-none">
            <MailWarning className="w-4 h-4 mr-2 text-cyber-blue-500 animate-bounce" />
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-cyber-dark-border text-xs">
                {tickets.map(t => (
                  <tr key={t.id} className="hover:bg-cyber-dark-border/20 transition-all">
                    <td className="py-3 px-4 text-gray-500 font-mono select-none">
                      {new Date(t.created_at).toLocaleDateString()}
                    </td>
                    <td className="py-3 px-4 py-4 max-w-[280px]">
                      <div className="font-bold text-white leading-tight">{t.subject}</div>
                      <div className="text-[10px] text-gray-400 mt-1 leading-relaxed whitespace-pre-wrap">{t.message}</div>
                    </td>
                    <td className="py-3 px-4 select-all">
                      <div className="font-semibold text-white">{t.name}</div>
                      <div className="text-[10px] text-gray-500 font-mono mt-0.5">{t.email}</div>
                    </td>
                    <td className="py-3 px-4 font-mono select-none">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border ${
                        t.status === 'open' 
                    ...
                    <td className="py-3 px-4 text-right select-none">
                      {t.status === 'open' ? (
                        <button
                          ...
                          className="px-2.5 py-1 bg-cyber-blue-900/40 hover:bg-cyber-blue-800/40 border border-cyber-blue-700/30 rounded text-[10px] text-cyber-blue-400 transition"
                        >
                          Mark Resolved
                        </button>
                      ) : (
      ...
      {adminTab === 'logs' && (
        <GlassCard hoverable={false} className="p-6">
          <h3 className="text-sm font-bold text-white font-mono uppercase tracking-wider mb-6 pb-2 border-b border-cyber-dark-border flex items-center select-none">
            <Terminal className="w-4 h-4 mr-2 text-cyber-blue-500" />
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-cyber-dark-border text-xs font-mono">
                {logs.map(log => (
                  <tr key={log.id} className="hover:bg-cyber-dark-border/20 transition-all text-[11px] text-gray-300">
                    <td className="py-3 px-4 text-gray-500 select-none">
                      {new Date(log.created_at).toLocaleString()}
                    </td>
                    <td className="py-3 px-4 font-semibold text-white select-all">{log.actor_name}</td>
                    <td className="py-3 px-4 text-cyber-blue-400 select-none uppercase text-[10px] font-bold">{log.action}</td>
                    <td className="py-3 px-4 text-gray-400 select-none">{log.details}</td>
                    <td className="py-3 px-4 text-right text-gray-500 select-all">{log.ip_address}</td>
                  </tr>
                ))}
              </tbody>
            </table>
